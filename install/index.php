...
$tokyo_install_mutating_actions = ['auth', 'config_write', 'bootstrap', 'probe_begin', 'finish'];
if (in_array($action, $tokyo_install_mutating_actions, true) && ($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST') {
    tokyo_install_json(['error' => 'POST required'], 405);
}
...
if ($action !== '') {
    ...
    if ($action === 'auth') {
        ...
        if (!$matches) {
            usleep(700000);
            tokyo_install_json(['ok' => false, 'error' => 'توکن ربات نادرست است.'], 403);
        }
    ...
    if ($action === 'finish') {
        ...
        @file_put_contents(tokyo_install_lock_file(), (string) time());
